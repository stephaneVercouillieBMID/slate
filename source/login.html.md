...
**state** | Strongly RECOMMENDED | This `state` object is the only way to bind the original Authentication Request with the Authorization Response (holding the `code` and the `state`). In other terms, this parameter is used to restore the user context on your side.
