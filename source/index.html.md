...
<aside class="notice">In order to avoid tampering, you can use the <code>request</code> or <code>request_URI</code> parameter in the Authentication Request. This enables sending Authentication Request parameters in a self-contained JWT instead of plain request parameters.</aside>
...
**request** | Optional | This parameter enables OpenID Connect requests to be passed in a single, self-contained parameter and to be signed with your private key and/or encrypted with the itsme® public key. The parameter value is a Request Object value. It represents the request as a JWT whose claims are the request parameters.<br>See [Appendixes](#RequestObjectByValue) for more information.</br><br>Note: when using encrypted JWT in the Authentication Request, this will yield very long requests. To avoid any problem and still use the GET method, please use the <code>request_uri</code> parameter instead.</br>
**request_uri** | Optional | This parameter enables OpenID Connect requests to be passed by reference, rather than by value. The request_uri value is a URL using the https scheme referencing a resource containing a Request Object value, which is a JWT containing the request parameters.<br>See <a href="#RequestUri">Using request_uri parameter</a> for more details.</br>
...
The `request` parameter enables the Authentication Requests to be passed in a single, self-contained parameter and to be signed and encrypted. It represents the request as a JWT whose claims are the parameters specified in section 3.2. This JWT is called a Request Object by value.
...
<ol>
  ...
  <li>The Request Object MUST be signed using the JSON Web Signature (JWS). The Request Object SHOULD contain the claims <code>iss</code> (issuer) and <code>aud</code> (audience) as members. The <code>iss</code> value SHOULD be your Client ID. The <code>aud</code> value SHOULD be or include the itsme®'s Issuer Identifier URL: https://merchant.itsme.be/oidc.</li>
  <li>The Request Object MUST also be encrypted using JSON Web Encryption (JWE). It MUST be signed then encrypted, with the result being a Nested JWT, as defined in the <a href=" https://tools.ietf.org/html/rfc7519" target="blank"> JSON Web Token (JWT) specification</a>.</li>
