...
**prompt** | Optional | OPTIONAL but not recommended. Only `login` and `consent` values are supported. If prompt contains `none` or `select_account` or any other values an HTTP ERROR `not_implemented` will be returned.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgxNDg2NDE0N119
-->